...
jQuery(document).ready(function() {
	var parnnterdahsbaordajax = '<?php echo admin_url('admin-ajax.php');?>';
	/******************Partners submit********************/
	...
	jQuery('#partner_save_btn').click(function(){
		...
		if(partnername ==''){
			...
			jQuery('body').find('#partner_save_btn').prop('disabled',true);
			jQuery('body').find('#partner_save_btn').attr('style','border-color: #ccc;background-color: #c1c6d0!important;color: #fff!important;');
			return false;
		} else {
	...
	jQuery('body').on('click','#save_editpartner', function(e) {
		...
		jQuery.ajax({
        	...
        	beforeSend: function(){
			    jQuery(' #save_editpartner').parent().find("#loader_imagedv_save_loader").show();
			    jQuery('#save_editpartner').addClass('submit_disable');
			},
	...
	jQuery('#add_partner_input').keyup(function(){
		if(jQuery(this).val()==''){
			...
			jQuery('body').find('#partner_save_btn').prop('disabled',false);
			jQuery('body').find('#partner_save_btn').removeAttr('style');	
		}
	});
}); //document end
</script>